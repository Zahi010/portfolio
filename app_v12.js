  ...
  gsap.to(playerGroup.position, {
    ...
    onComplete: () => {
      ...
      if (gameState.warpTunnel) {
        ...
        if (gameState.warpStars) {
          gameState.warpStars.geometry.dispose();
          gameState.warpStars.material.dispose();
        }
        if (gameState.warpRings) {
...
function buildWarpTunnel() {
  ...
  tunnelGroup.position.set(0, -150, 0);

  // 1. A long cylinder of wireframe data that rushes past the player
  ...
  // 2. High-speed warp stars (streaking particles)
  ...
  const starMat = new THREE.PointsMaterial({ color: 0xffffff, size: 0.25, transparent: true, opacity: 0.9 });
  ...
  const ringMat = new THREE.MeshBasicMaterial({ color: 0xaa00ff, transparent: true, opacity: 0.6 });
  for(let i=0; i<ringCount; i++) {
    const ring = new THREE.Mesh(ringGeo, ringMat);
    ring.position.y = (Math.random() - 0.5) * 300;
  ...
  try {
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.warpTunnel && gameState.isDropping) {
        ...
        gameState.warpTunnel.position.y += 400 * dt; // Much faster!
        gameState.warpCylinder.rotation.y += 5 * dt;   

        // Stars streak incredibly fast relative to the tunnel
        const starPositions = gameState.warpStars.geometry.attributes.position.array;
        for(let i=1; i<starPositions.length; i+=3) {
          starPositions[i] += 1200 * dt; 
          if (starPositions[i] > 150) starPositions[i] = -150;
        }
        gameState.warpStars.geometry.attributes.position.needsUpdate = true;

        // Rings move rapidly
        gameState.warpRings.children.forEach(ring => {
          ring.position.y += 800 * dt;
          if (ring.position.y > 150) ring.position.y = -150;
        });
