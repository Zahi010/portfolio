...
function updateMovement() {
  ...
  if (gameState.isDropping) {
    if (gameState.warpCurve) {
       ...
       const pt = gameState.warpCurve.getPointAt(gameState.warpProgress);
       
       // 2. Allow user to steer inside the wormhole!
       let dropSpeed = 1.0;
       let dx = 0, dz = 0;
       if (gameState.keys.w || gameState.keys.ArrowUp) { dz -= dropSpeed; }
       if (gameState.keys.s || gameState.keys.ArrowDown) { dz += dropSpeed; }
       if (gameState.keys.d || gameState.keys.ArrowRight) { dx += dropSpeed; }
       if (gameState.keys.a || gameState.keys.ArrowLeft) { dx -= dropSpeed; }
       
       gameState.dropOffsetX += dx;
       gameState.dropOffsetZ += dz;

       // Keep player inside the wormhole tube (Radius is 35, limit to 25)
       const dist = Math.sqrt(gameState.dropOffsetX**2 + gameState.dropOffsetZ**2);
       if (dist > 25) {
           const angle = Math.atan2(gameState.dropOffsetZ, gameState.dropOffsetX);
           gameState.dropOffsetX = Math.cos(angle) * 25;
           gameState.dropOffsetZ = Math.sin(angle) * 25;
       }

       // 3. Update player position on the curve + manual offsets
       playerGroup.position.copy(pt);
       playerGroup.position.x += gameState.dropOffsetX;
       playerGroup.position.z += gameState.dropOffsetZ;

       // 4. Orient player along the curve!
       if (gameState.warpProgress < 0.99) {
           const lookPt = gameState.warpCurve.getPointAt(gameState.warpProgress + 0.01);
           playerGroup.lookAt(lookPt.x + gameState.dropOffsetX, lookPt.y, lookPt.z + gameState.dropOffsetZ);
       }

       // Epic Third Person Falling Camera!
       camera.position.x = playerGroup.position.x;
       camera.position.y = playerGroup.position.y + 6; // Over the shoulder
       camera.position.z = playerGroup.position.z + 8; // Just behind
       
       // Look further down the curve
       if (gameState.warpProgress < 0.95) {
           const camLook = gameState.warpCurve.getPointAt(gameState.warpProgress + 0.05);
           camera.lookAt(camLook);
       }

       // Smoothly bank and lean the character model for that "flying" feel!
       const model = playerGroup.children.find(c => c.type === 'Group' && c !== gameState.warpTunnel);
       if (model) {
           const rawX = (gameState.keys.d || gameState.keys.ArrowRight) ? 1 : ((gameState.keys.a || gameState.keys.ArrowLeft) ? -1 : 0);
           const rawZ = (gameState.keys.s || gameState.keys.ArrowDown) ? 1 : ((gameState.keys.w || gameState.keys.ArrowUp) ? -1 : 0);
           const targetBank = -rawX * 0.4;
           const targetPitch = rawZ * 0.4;
