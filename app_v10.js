...
function buildSkyExtras() {
  ...
  const ringMat2 = new THREE.MeshBasicMaterial({ color: 0xaa00ff, transparent: true, opacity: 0.15 });
  ...
  const hubMat = new THREE.MeshBasicMaterial({ color: 0x00f3ff, wireframe: true, transparent: true, opacity: 0.4 });
  for(let i=0; i<3; i++) {
  ...
  for(let i=0; i<2; i++) {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(2, 2, 4), new THREE.MeshBasicMaterial({ color: 0x555555 }));
    satGroup.add(body);
    // Solar panels
    const panelGeo = new THREE.PlaneGeometry(8, 2);
    const panelMat = new THREE.MeshBasicMaterial({ color: 0x0077ff, wireframe: true });
    const panel = new THREE.Mesh(panelGeo, panelMat);
    panel.rotation.x = Math.PI / 2;
    satGroup.add(panel);
    
    // Laser beam (pointing local -Z)
    const laserMat = new THREE.LineBasicMaterial({ color: 0xff0055, transparent: true, opacity: 0.5 });
  ...
  gameState.auroras = [];
  const auroraMat = new THREE.MeshBasicMaterial({ color: 0xaa00ff, transparent: true, opacity: 0.1, side: THREE.DoubleSide, blending: THREE.AdditiveBlending });
  for(let i=0; i<3; i++) {
