...
function buildWarpTunnel() {
  ...
  const tubeRingGeo = new THREE.TorusGeometry(20, 0.5, 4, 32);
  ...
  const lightnings = [];
  const plasmaGeo = new THREE.SphereGeometry(15, 8, 8);
  for(let i=0; i<8; i++) {
    const plasmaMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0 });
