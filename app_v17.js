...
function buildWarpTunnel() {
  ...
  // 1. A twisting spiraling wormhole instead of a straight cylinder
  class WormholeCurve extends THREE.Curve {
    constructor() { super(); }
    getPoint(t, optionalTarget = new THREE.Vector3()) {
      // t goes from 0 to 1
      const twistRadius = 4; // Tight twist so stars stay inside
      const length = 2000;
      const twists = 5;
      
      const y = (t - 0.5) * length; 
      const theta = t * Math.PI * 2 * twists;
      const x = Math.cos(theta) * twistRadius;
      const z = Math.sin(theta) * twistRadius;
      return optionalTarget.set(x, y, z);
    }
  }

  const path = new WormholeCurve();
  const geo = new THREE.TubeGeometry(path, 100, 15, 16, false); // Radius 15 to easily fit rings
  ...
  const lightnings = [];
  for(let i=0; i<8; i++) {
    const pLight = new THREE.PointLight(0xffffff, 0, 100);
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.warpTunnel && gameState.isDropping) {
        ...
        gameState.warpLightnings.forEach(l => {
          if (Math.random() > 0.85) {
            l.intensity = 20 + Math.random() * 80;
            l.position.y = (Math.random()-0.5)*2000; // Jump around
          } else {
            l.intensity *= 0.7; // fade quickly
          }
        });
      }
