...
function buildWarpTunnel() {
  ...
  // 1. A twisting spiraling wormhole using stacked rings (guaranteed to render)
  const tubeGroup = new THREE.Group();
  const tubeRingGeo = new THREE.TorusGeometry(20, 2.0, 8, 32); // Thick, solid rings
  const tubeRingMat = new THREE.MeshBasicMaterial({ 
    color: 0x00f3ff, 
    wireframe: false, 
    transparent: true, 
    opacity: 0.5,
  ...
  // Space them closely to form a ribbed tunnel
  for(let y=-1000; y<=1000; y+= 20) {
    const mesh = new THREE.Mesh(tubeRingGeo, tubeRingMat);
    mesh.position.y = y;
    mesh.position.x = Math.sin(y * 0.005) * 16;
    mesh.position.z = Math.cos(y * 0.005) * 16;
    mesh.rotation.x = Math.PI / 2; // Flat relative to the fall
    tubeGroup.add(mesh);
  }
  ...
  // 4. Plasma Lightning (Solid glowing spheres instead of lights, for 100% visibility)
  const lightnings = [];
  const plasmaGeo = new THREE.SphereGeometry(25, 8, 8); // Huge spheres
  for(let i=0; i<8; i++) {
    // CRITICAL: DoubleSide ensures they are visible even if the camera drops inside them!
    const plasmaMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0, side: THREE.DoubleSide });
    const mesh = new THREE.Mesh(plasmaGeo, plasmaMat);
    mesh.position.set((Math.random()-0.5)*12, (Math.random()-0.5)*2000, (Math.random()-0.5)*12);
    tunnelGroup.add(mesh);
    lightnings.push({ mesh: mesh, active: false, timer: 0 });
  }
  ...
  gameState.warpLightnings = lightnings;
}
  ...
  try {
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.warpTunnel && gameState.isDropping) {
        // Spin the tube group SLOWLY so the twisting shape is visible
        gameState.warpTubeGroup.rotation.y += 1.5 * dt;   

        // Move the tube rings UP relative to the player so they fly through the tunnel!
        gameState.warpTubeGroup.children.forEach(ring => {
           ring.position.y += 2000 * dt; // Rushing upwards
