...
window.enterMissionControl = function enterMissionControl() {
  ...
  if (gameState.glitchPass) {
    gameState.glitchPass.enabled = true;
    gameState.glitchPass.goWild = false;
    // Make it go completely haywire just before impact
    setTimeout(() => {
      if (gameState.glitchPass) gameState.glitchPass.goWild = true;
    }, 4500); 
  }
  ...
  gsap.to(playerGroup.position, {
    ...
    onComplete: () => {
      ...
      if (gameState.glitchPass) {
        gameState.glitchPass.enabled = false;
        gameState.glitchPass.goWild = false;
      }
...
function buildWarpTunnel() {
  ...
  // 1. A twisting spiraling wormhole using vertex displacement
  const geo = new THREE.CylinderGeometry(20, 20, 2000, 32, 100, true);
  
  // Physically bend the straight cylinder into a wavy spiral
  ...
  for (let i = 0; i < pos.length; i += 3) {
    ...
    // Displace center based on Y to create a sine-wave spiral
    const dx = Math.sin(y * 0.005) * 16;
    const dz = Math.cos(y * 0.005) * 16;
  ...
  const mat = new THREE.MeshStandardMaterial({
    color: 0x111111,        // Dark base color so lightning illuminates it
    emissive: 0x001133,     // Deep blue ambient glow
    emissiveIntensity: 0.1, // Low emissive so lightning contrast is huge
    wireframe: false,       // Solid so lights actually catch it
    transparent: true,
    opacity: 0.9,
  ...
  const ringGeo = new THREE.TorusGeometry(7.5, 0.1, 8, 32);
  ...
  const lightnings = [];
  for(let i=0; i<8; i++) {
    const pLight = new THREE.PointLight(0xffffff, 0, 800); // Huge distance to light up tunnel
    pLight.position.set((Math.random()-0.5)*12, (Math.random()-0.5)*2000, (Math.random()-0.5)*12);
    tunnelGroup.add(pLight);
    lightnings.push(pLight);
  }
  ...
  gameState.warpCylinder = cylinder;
...
function init3D() {
  ...
  if (typeof THREE.EffectComposer !== 'undefined') {
    ...
    if (typeof THREE.GlitchPass !== 'undefined') {
      gameState.glitchPass = new THREE.GlitchPass();
      gameState.glitchPass.goWild = false;
      gameState.glitchPass.enabled = false;
      gameState.composer.addPass(gameState.glitchPass);
    }
  }
  ...
  try {
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.warpTunnel && gameState.isDropping) {
        // Spin the cylinder SLOWLY so the twisting shape is visible
        gameState.warpCylinder.rotation.y += 1.5 * dt;   
        ...
        gameState.warpLightnings.forEach(l => {
          if (Math.random() > 0.85) {
            l.intensity = 50 + Math.random() * 200; // Extremely bright flashes
            l.position.y = (Math.random()-0.5)*2000; // Jump around
          } else {
            l.intensity *= 0.5; // fade quickly
          }
        });
      }
