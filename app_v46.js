...
function playSound(type) {
  ...
  if (type === 'boot') {
    ...
    osc.frequency.linearRampToValueAtTime(100, audioCtx.currentTime + 6.0); // pitch up as we fall
    gain.gain.setValueAtTime(0, audioCtx.currentTime);
    gain.gain.linearRampToValueAtTime(0.3, audioCtx.currentTime + 1.0);
    gain.gain.setValueAtTime(0.3, audioCtx.currentTime + 5.5);
    gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 6.0);
    osc.connect(gain);
    gain.connect(dest);
    osc.start();
    ...
    filter.frequency.exponentialRampToValueAtTime(3000, audioCtx.currentTime + 2.0);
    const gain = audioCtx.createGain();
    gain.gain.setValueAtTime(0, audioCtx.currentTime);
...
function runBootSequence() {
  ...
  document.getElementById('boot-screen').addEventListener('click', () => {
    ...
    gsap.to(overlay, { opacity: 0, duration: 1, onComplete: () => {
      ...
      enterZeroGSpace(); // Go to Zero G first!
    }});
  }, { once: true });
}
...
function enterZeroGSpace() {
  gameState.isBooted = true;
  gameState.isZeroG = true;
  gameState.missionCompleted = false;
  gameState.gravityPull = 0; // Initialize gravity pull

  // Move character to isolated Deep Space Dimension
  const DEEP_SPACE_COORD = 50000;
  playerGroup.position.set(DEEP_SPACE_COORD, DEEP_SPACE_COORD, DEEP_SPACE_COORD);
  playerGroup.rotation.set(0, 0, 0);
  
  // Disable fog and increase render distance for Deep Space
  ...
  // Set camera manually looking slightly around
  gameState.yaw = 0;
  gameState.pitch = 0;

  // Play idle or floating animation
  if (isModelLoaded && idleAction) {
    setAction(idleAction);
  }
  
  // Generate Local Deep Space Starfield (Multi-colored)
  ...
  const colorPalette = [
     ...
     new THREE.Color(0xaaaaFF), // Blueish
     new THREE.Color(0xffccaa), // Orange/Yellow
     new THREE.Color(0x444444)  // Faint
  ];

  for(let i=0; i<starCount*3; i+=3) {
    const radius = 500 + Math.random() * 1500; // further out
    const theta = Math.random() * Math.PI * 2;
    const phi = Math.acos((Math.random() * 2) - 1);
    starPos[i]   = DEEP_SPACE_COORD + radius * Math.sin(phi) * Math.cos(theta);
    starPos[i+1] = DEEP_SPACE_COORD + radius * Math.sin(phi) * Math.sin(theta);
    starPos[i+2] = DEEP_SPACE_COORD + radius * Math.cos(phi);
  ...
  const starMat = new THREE.PointsMaterial({size: 2.0, transparent: true, opacity: 0.8, vertexColors: true});
  gameState.zeroGStars = new THREE.Points(starGeo, starMat);
  scene.add(gameState.zeroGStars);

  // --- REALISTIC SOLAR SYSTEM PROPS ---
  gameState.solarSystem = new THREE.Group();
  // Move the system much further away so the player can see the whole thing like a diorama
  gameState.solarSystem.position.set(DEEP_SPACE_COORD, DEEP_SPACE_COORD, DEEP_SPACE_COORD - 1200); 
  scene.add(gameState.solarSystem);
  
  // 1. The Sun
  const sunRadius = 80;
  const sunGeo = new THREE.SphereGeometry(sunRadius, 64, 64);
  const sunMat = new THREE.MeshBasicMaterial({ color: 0xffaa00 });
  gameState.zeroGSun = new THREE.Mesh(sunGeo, sunMat);
  gameState.solarSystem.add(gameState.zeroGSun);
  
  // Sun Light - Keeps the area illuminated
  const sunLight = new THREE.PointLight(0xffeedd, 2.0, 5000); 
  gameState.zeroGSun.add(sunLight);
  
  // Ambient Light
  const ambientLight = new THREE.AmbientLight(0xffffff, 1.0);
  gameState.solarSystem.add(ambientLight);
  
  // Sun Corona (Slightly larger, additive blending)
  const coronaGeo = new THREE.SphereGeometry(sunRadius * 1.05, 64, 64);
  const coronaMat = new THREE.MeshBasicMaterial({ color: 0xff3300, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending, depthWrite: false });
  const sunCorona = new THREE.Mesh(coronaGeo, coronaMat);
  gameState.zeroGSun.add(sunCorona);

  // Sun Aura (Massive glow)
  const sunAuraGeo = new THREE.SphereGeometry(sunRadius * 1.5, 64, 64);
  const sunAuraMat = new THREE.MeshBasicMaterial({ color: 0xff4400, transparent: true, opacity: 0.2, blending: THREE.AdditiveBlending, depthWrite: false });
  const sunAura = new THREE.Mesh(sunAuraGeo, sunAuraMat);
  gameState.zeroGSun.add(sunAura);
  
  // 2. Orbiting Planets
  gameState.zeroGPlanets = [];
  
  const createPlanet = (radius, distance, speed, colorHex, hasRings = false, ringColor = 0xffffff) => {
     // Orbit Line
     ...
     orbit.rotation.x = Math.PI / 2; // Flat on XZ plane
     gameState.solarSystem.add(orbit);
     
     // Planet Mesh - Using BasicMaterial guarantees vibrant colors without lighting issues
     const pGeo = new THREE.SphereGeometry(radius, 32, 32);
     const pMat = new THREE.MeshBasicMaterial({ color: colorHex });
     const pMesh = new THREE.Mesh(pGeo, pMat);
     
     // Add rings if Gas Giant
     if (hasRings) {
         const rGeo = new THREE.RingGeometry(radius * 1.4, radius * 2.2, 64);
         const rMat = new THREE.MeshBasicMaterial({ color: ringColor, transparent: true, opacity: 0.7, side: THREE.DoubleSide });
         const rMesh = new THREE.Mesh(rGeo, rMat);
         rMesh.rotation.x = Math.PI / 2 + 0.2; // Tilted rings
         pMesh.add(rMesh);
     }
     
     gameState.solarSystem.add(pMesh);
     
     gameState.zeroGPlanets.push({
         mesh: pMesh,
         orbitLine: orbit,
         distance: distance,
         speed: speed,
  ...
  // Planet 1: Hot Rocky Planet (Red/Orange)
  createPlanet(10, 150, 0.4, 0xff4422);
  
  // Planet 2: Habitable Water Planet (Blue/Green)
  createPlanet(15, 250, 0.2, 0x2288ff);
  
  // Planet 3: Massive Gas Giant with Rings (Yellow/Brown)
  createPlanet(35, 400, 0.08, 0xffcc88, true, 0xffeecc);

  // 3. Dynamic Meteors
  gameState.zeroGMeteors = [];
  const meteorGeo = new THREE.DodecahedronGeometry(3, 0);
  const meteorMat = new THREE.MeshBasicMaterial({ color: 0x888888 });
  for (let i = 0; i < 15; i++) {
     ...
     // Trail
     const trailGeo = new THREE.CylinderGeometry(0.1, 2, 20, 8);
     trailGeo.translate(0, -10, 0);
     trailGeo.rotateX(Math.PI / 2); // Point backward
     const trailMat = new THREE.MeshBasicMaterial({ color: 0xffaa00, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending });
     const trail = new THREE.Mesh(trailGeo, trailMat);
     mGroup.add(trail);
     
     // Hide initially, handled in animate()
     mGroup.position.set(0, -10000, 0);
     mGroup.userData = { active: false };
     gameState.solarSystem.add(mGroup);
     gameState.zeroGMeteors.push(mGroup);
  }
  ...
  if (prompt) {
    ...
    prompt.innerHTML = `ANOMALY DETECTED IN LOCAL STAR - INVESTIGATE WITH <span>WASD</span>`;
  }

  // --- THE SUPERNOVA TIMELINE ---
  const timeline = gsap.timeline({delay: 14.0}); // Player floats freely for 14s
  
  // Phase 1: Destabilize
  timeline.add(() => {
     playSound('beep'); // Warning sound
     gameState.shakeAmount = 1.0; // Mild rumble
     gsap.to(sunAura.scale, { x: 1.5, y: 1.5, z: 1.5, duration: 1.5, yoyo: true, repeat: -1 });
     gsap.to(sunCorona.scale, { x: 1.2, y: 1.2, z: 1.2, duration: 0.5, yoyo: true, repeat: -1 });
     gsap.to(sunMat.color, { r: 1, g: 1, b: 1, duration: 2.0 }); // Shift to white
     gsap.to(sunAuraMat.color, { r: 0, g: 0.5, b: 1, duration: 2.0 }); // Shift to blue
  });
  
  // Phase 2: Supernova Flash (t=16.0s)
  timeline.to({}, { duration: 2.0 }); // Wait 2s during destabilization
  timeline.add(() => {
     gameState.shakeAmount = 4.0; // Massive shake
     
     // Blinding Flash
     const flash = new THREE.AmbientLight(0xffffff, 100.0);
     scene.add(flash);
     gsap.to(flash, { intensity: 0, duration: 2.0, ease: "power2.out", onComplete: () => {
         scene.remove(flash);
     }});
     
     // Destroy Sun
     gameState.solarSystem.remove(gameState.zeroGSun);
     gameState.zeroGSun.geometry.dispose();
     gameState.zeroGSun.material.dispose();
     gameState.zeroGSun = null;
     
     // Phase 3: The Massive Wormhole Appears!
     // Create Wormhole Entrance Marker
     const whGeo = new THREE.SphereGeometry(150, 32, 32); // Massive!
     const whMat = new THREE.MeshBasicMaterial({ color: 0x00f3ff, wireframe: true, transparent: true, opacity: 0.5 });
     gameState.wormholeEntrance = new THREE.Mesh(whGeo, whMat);
     
     // Position exactly where the Sun was (local to solarSystem group)
     gameState.wormholeEntrance.position.set(0, 0, 0); 
     gameState.wormholeEntrance.scale.set(0, 0, 0);
     gameState.solarSystem.add(gameState.wormholeEntrance);

     // Glowing aura
     const auraGeo = new THREE.SphereGeometry(200, 32, 32);
     const auraMat = new THREE.MeshBasicMaterial({ color: 0x00aaff, transparent: true, opacity: 0.2, blending: THREE.AdditiveBlending, depthWrite: false });
     gameState.wormholeAura = new THREE.Mesh(auraGeo, auraMat);
     gameState.wormholeEntrance.add(gameState.wormholeAura);

     // Animate opening
     gsap.to(gameState.wormholeEntrance.scale, { x: 1, y: 1, z: 1, duration: 1.0, ease: "elastic.out(1, 0.5)" });
     
     // Ramp up Gravitational Pull!
     gsap.to(gameState, { gravityPull: 5.0, duration: 8.0, ease: "power2.in" });

     if (prompt) {
       prompt.innerHTML = `GRAVITATIONAL COLLAPSE IMMINENT - <span>BRACE FOR IMPACT</span>`;
     }
  });
}

window.enterMissionControl = function enterMissionControl() {
  gameState.isZeroG = false;
  gameState.isDropping = true;
  gameState.gravityPull = 0;
  
  // Hide prompt
  const prompt = document.getElementById('interaction-prompt');
  if (prompt) prompt.style.opacity = 0;
  
  // Clean up Deep Space Dimension
  if (gameState.solarSystem) {
      scene.remove(gameState.solarSystem);
      // We don't necessarily need to meticulously dispose every geometry here since it's a one-off,
      // but we remove the whole group to clean the scene
      gameState.solarSystem = null;
  }
  if (gameState.zeroGStars) {
      scene.remove(gameState.zeroGStars);
      gameState.zeroGStars.geometry.dispose();
      gameState.zeroGStars.material.dispose();
      gameState.zeroGStars = null;
  }
  
  // Teleport player back to the REAL facility dimension!
  playerGroup.position.set(0, 150, -60);
  playerGroup.rotation.set(0, 0, 0);
  
  // Restore fog and normal render distance for the room
  ...
  // Hide boot screen quickly
  gsap.to("#boot-screen", { opacity: 0, duration: 0.2, onComplete: () => {
    document.getElementById('boot-screen').style.display = 'none';
  }});

  // 1. Lock controls and spawn player in the sky
  gameState.warpProgress = 0;
  gameState.dropOffsetX = 0;
  gameState.dropOffsetZ = 0;

  // 2. Build Warp Tunnel & Follow Light
  buildWarpTunnel();
  
  // Add an intense glowing light attached to the player so they are illuminated inside the dark shader tube
  const dropLight = new THREE.PointLight(0xffffff, 8.0, 50);
  dropLight.position.set(0, 5, 0); // Float slightly above the character
  playerGroup.add(dropLight);

  // 3. Ambient Hum
  startAmbientHum();

  // 4. Smoothly drop the player along the spline!
  gsap.to(gameState, {
    warpProgress: 1.0,
    duration: 14.0, // Doubled duration for a much longer drop!
    ease: "power2.in", // Accelerate downwards
    onComplete: () => {
      gameState.isDropping = false;
      gameState.isLanding = true;
      
      // Snap Character Upright mid-air
      playerGroup.up.set(0, 1, 0);
      playerGroup.rotation.set(0, 0, 0);  
      camera.up.set(0, 1, 0);
      
      // Clean up drop light and tunnel immediately so they fall into the open room
      playerGroup.remove(dropLight);
      if (gameState.warpTunnel) scene.remove(gameState.warpTunnel);
      gameState.warpTunnel = null;
      if (gameState.warpStars && gameState.warpStars.children.length > 0) {
          gameState.warpStars.children[0].geometry.dispose();
      }
      if (gameState.warpRings && gameState.warpRings.children.length > 0) {
          gameState.warpRings.children[0].geometry.dispose();
      }
      
      // Make the character do a front-flip while falling
      playerGroup.children.forEach(child => {
        child.visible = true;
        if (child.type === 'Group') {
           child.rotation.set(-Math.PI * 2, 0, 0); // 360 degree rotation to unwind
           gsap.to(child.rotation, { x: 0, duration: 1.5, ease: "power2.out" });
        }
      });
      
      // BALLISTIC THROW ANIMATION
      // Move horizontally linearly
      ...
      gsap.to(playerGroup.position, {
        ...
        onComplete: () => {
          ...
          gameState.isCinematicLying = true;
          gameState.cinematicPan = 0;
          gameState.cinematicLookUp = 100.0; // Start by staring straight up into the sky!
          gameState.cinematicCameraBlend = 1.0; // 1.0 = full first person

          // The character needs to physically lay on the ground
          const model = playerGroup.children.find(c => c.type === 'Group');
          if (model) {
             model.rotation.set(-Math.PI / 2, 0, 0); // Lay flat on back!
          }

          if (gameState.rgbShift) {
            gameState.rgbShift.enabled = false;
            gameState.rgbShift.uniforms['amount'].value = 0.0;
          }
          
          // Heavy Camera Shake
          gameState.shakeAmount = 4.0;
    
          // Eyelids closed instantly
          const lids = document.getElementById('eye-lids');
          if (lids) lids.style.opacity = '1';
          
          // Eyelids blink open sequence
          const blinkTimeline = gsap.timeline({delay: 1.0}); // wait a bit after impact in darkness
          
          // Blink 1
          blinkTimeline.to(lids, { opacity: 0.3, duration: 0.2, ease: "power1.inOut" });
          blinkTimeline.to(lids, { opacity: 1.0, duration: 0.3, ease: "power1.inOut" });
          
          // Blink 2
          blinkTimeline.to(lids, { opacity: 0.1, duration: 0.3, ease: "power1.inOut", delay: 0.5 });
          blinkTimeline.to(lids, { opacity: 0.8, duration: 0.2, ease: "power1.inOut" });
          
          // Fully Open
          blinkTimeline.to(lids, { opacity: 0, duration: 1.5, ease: "power2.inOut", delay: 0.3 });
          
          // Slowly bring gaze down from the sky
          gsap.to(gameState, {
             cinematicLookUp: 3.0,
             duration: 3.0,
             ease: "power2.inOut",
             delay: 2.0 // Start lowering head as eyes open
          });
          
          // Pan camera around sequence
          gsap.to(gameState, { 
             cinematicPan: Math.PI * 2, // full 360 rotation looking around the room
             duration: 6.0, 
             ease: "power1.inOut",
             delay: 3.5, // Start panning around after lowering gaze slightly
             onComplete: () => {
                // Stand up sequence
                if (model) gsap.to(model.rotation, { x: 0, duration: 1.5, ease: "back.out(1.2)" }); // Character stands up!
                
                // Transition camera back to Third Person seamlessly!
                gsap.to(gameState, {
                   cinematicCameraBlend: 0.0, // blend to 0.0 for Third Person
                   duration: 1.5,
                   ease: "power2.inOut",
                   onComplete: () => {
                      gameState.isCinematicLying = false;
                      
                      // BOOM! Power on the entire facility as soon as they stand up!
                      startAmbientHum();
                      ...
                      gsap.to(flash, { intensity: 0, duration: 0.5, ease: "power2.out", onComplete: () => {
                        ...
                        if (gameState.pulses) {
                          gameState.pulses.forEach(p => {
                            ...
                            let delayDistance = (angle / (Math.PI * 2)) * 30;
...
function buildWarpTunnel() {
  ...
  // 1. Winding Spline Path (Doubled Length)
  const points = [];
  let currentY = 2400; // Start twice as high
  for(let i=0; i<=40; i++) {
     // Create a winding path that twists around
     const x = Math.sin(i * 0.6) * 80;
     const z = Math.cos(i * 0.4) * 80;
     points.push(new THREE.Vector3(x, currentY, z));
     currentY -= 60;
  }
  // Ensure the last few points straighten out and point down towards the room
  points[38] = new THREE.Vector3(0, 150, -60);
  points[39] = new THREE.Vector3(0, 100, -45);
  points[40] = new THREE.Vector3(0, 60, -30); // Stop mid-air so player falls out!

  const curve = new THREE.CatmullRomCurve3(points);
  gameState.warpCurve = curve;

  // 2. Custom Shader Material for Swirling Plasma
  const vertexShader = `
    varying vec2 vUv;
    void main() {
      vUv = uv;
  ...
  const fragmentShader = `
    ...
    void main() {
      // Stretch UVs to make speed lines (y is along the tube, x is around it)
      vec2 st = vUv;
      st.x *= 8.0; // Wrap around
      st.y *= 150.0; // Stretch long
      
      // Add time to simulate rushing forward and swirling
      st.y += time * 40.0; // Extreme forward speed
      st.x += sin(time * 0.5) * 2.0;

      float n = noise(st);
      
      // Enhance contrast for sharp plasma streaks
      n = smoothstep(0.4, 0.8, n);
      
      // Dual-tone palette based on angle (vUv.x goes 0 to 1 around the tube)
      vec3 warmColor = vec3(1.0, 0.2, 0.6); // Pink/Red
      vec3 coolColor = vec3(0.0, 0.8, 1.0); // Cyan
      vec3 baseColor = mix(warmColor, coolColor, sin(vUv.x * 6.28 + time * 0.5) * 0.5 + 0.5);

      vec3 finalColor = baseColor * n * 3.0; // Boost brightness
  ...
  const tubeGeo = new THREE.TubeGeometry(curve, 200, 35, 32, false);
  const tunnelMesh = new THREE.Mesh(tubeGeo, tunnelMat);
  tunnelGroup.add(tunnelMesh);

  // 3. Floating Space Dust (Particles inside the tube)
  const dustGeo = new THREE.BufferGeometry();
  const dustCount = 5000; // More dust for the longer tube
  ...
  for(let i=0; i<dustCount; i++) {
     ...
     const r = Math.random() * 30; // Radius 30
     dustPos[i*3] = pt.x + Math.cos(angle) * r;
     dustPos[i*3+1] = pt.y + (Math.random() - 0.5) * 20;
     dustPos[i*3+2] = pt.z + Math.sin(angle) * r;
  }
  dustGeo.setAttribute('position', new THREE.BufferAttribute(dustPos, 3));
  const dustMat = new THREE.PointsMaterial({ color: 0xffffff, size: 0.5, transparent: true, opacity: 0.8 });
  const dustSystem = new THREE.Points(dustGeo, dustMat);
  tunnelGroup.add(dustSystem);

  // Add to scene so it exists in world space!
  scene.add(tunnelGroup);
  
  gameState.warpTunnel = tunnelGroup;
  gameState.warpTunnelMat = tunnelMat;
  gameState.warpDust = dustSystem;
}
...
const gameState = {
  ...
  isDropping: false,
  isLanding: false,
  isCinematicLying: false,
  cinematicPan: 0,
  cinematicLookUp: 0,
  cinematicCameraBlend: 0,
  shakeAmount: 0
};
...
function init3D() {
  ...
  if (typeof THREE.EffectComposer !== 'undefined') {
    gameState.composer = new THREE.EffectComposer(renderer);
    const renderPass = new THREE.RenderPass(scene, camera);
    gameState.composer.addPass(renderPass);
    
    gameState.rgbShift = new THREE.ShaderPass(THREE.RGBShiftShader);
    gameState.rgbShift.uniforms['amount'].value = 0.0;
    gameState.rgbShift.enabled = false;
    gameState.composer.addPass(gameState.rgbShift);
  }
...
function buildTerminals() {
  ...
  terminalData.forEach(data => {
    ...
    const screenMat = new THREE.MeshBasicMaterial({ color: 0x00f3ff, transparent: true, opacity: 0.0, side: THREE.DoubleSide });
    ...
    markerEl.style.opacity = '0'; // keep invisible until turned on
...
function activateEnvironment() {
  ...
  terminals.forEach(t => {
    if (t.userData.light && t.parent) {
      ...
      if (t.userData.markerEl) {
        gsap.to(t.userData.markerEl, { opacity: 1, duration: 0.5, delay: delay });
      }
    }
  });
...
function updateMovement() {
  ...
  // Zero-G Flight Phase
  if (gameState.isZeroG) {
      let speed = gameState.moveSpeed * 2.5; // Faster in space
      if (gameState.keys.Shift) speed *= gameState.sprintMultiplier;

      // True 3D Flight: Forward vector incorporates Pitch and Yaw
      const forward = new THREE.Vector3(
          Math.sin(gameState.yaw) * Math.cos(gameState.pitch),
          -Math.sin(gameState.pitch),
          Math.cos(gameState.yaw) * Math.cos(gameState.pitch)
      ).normalize();

      // Right vector is horizontal
      const right = new THREE.Vector3(1, 0, 0).applyAxisAngle(new THREE.Vector3(0, 1, 0), gameState.yaw);

      let moveX = 0, moveY = 0, moveZ = 0;

      // W/S move along the 3D forward vector
      if (gameState.keys.w || gameState.keys.ArrowUp) { moveX -= forward.x; moveY -= forward.y; moveZ -= forward.z; }
      if (gameState.keys.s || gameState.keys.ArrowDown) { moveX += forward.x; moveY += forward.y; moveZ += forward.z; }
      
      // A/D move strictly horizontally (strafing)
      ...
      // Apply Gravitational Pull from Wormhole!
      if (gameState.wormholeEntrance && gameState.gravityPull > 0) {
          const whWorldPos = new THREE.Vector3();
          gameState.wormholeEntrance.getWorldPosition(whWorldPos);
          
          // Calculate direction from player to wormhole
          const pullDir = new THREE.Vector3().subVectors(whWorldPos, playerGroup.position);
          const distToWH = pullDir.length();
          pullDir.normalize();
          
          // Add gravity vector to movement
          // Gravity is stronger the closer you are!
          const gravityIntensity = gameState.gravityPull * (1.0 + (300 / Math.max(distToWH, 1.0)));
          moveX += pullDir.x * gravityIntensity;
          moveY += pullDir.y * gravityIntensity;
          moveZ += pullDir.z * gravityIntensity;
      }

      // Normalize & Move Player (Allow player to struggle, but gravity adds directly)
      if (moveX !== 0 || moveY !== 0 || moveZ !== 0) {
        ...
        playerGroup.position.y += moveY;
        playerGroup.position.z += moveZ;

        // Lean into the flight direction
        const model = playerGroup.children.find(c => c.type === 'Group');
        if (model) {
           model.rotation.x = -Math.PI / 4; // Superman flight pose
        }
      } else {
        const model = playerGroup.children.find(c => c.type === 'Group');
        if (model) {
           model.rotation.x += (0 - model.rotation.x) * 0.1; // return upright
        }
      }

      // Rotate player group to always face camera yaw
      playerGroup.rotation.y = gameState.yaw + Math.PI;

      // Check collision with Wormhole Entrance
      if (gameState.wormholeEntrance) {
          const whWorldPos = new THREE.Vector3();
          gameState.wormholeEntrance.getWorldPosition(whWorldPos);
          const dist = playerGroup.position.distanceTo(whWorldPos);
          if (dist < 100) { // Huge hit radius since the wormhole is massive
              enterMissionControl();
          }
      }

      // Standard Third Person Camera
      const distance = 10.0; // Further back in space
  ...
  // Third Person Camera during drop sequence
  if (gameState.isDropping) {
    if (gameState.warpCurve) {
       // 1. Get position along the curved spline
       let t = gameState.warpProgress;
       if (t > 0.99) t = 0.99; // Prevent out of bounds
       
       // Fix gimbal lock/flipping by using Frenet Frames for a stable Up vector
       if (!gameState.warpFrames) {
           gameState.warpFrames = gameState.warpCurve.computeFrenetFrames(200, false);
       }
       const frameIndex = Math.min(Math.floor(t * 200), 200);
       const normal = gameState.warpFrames.normals[frameIndex];
       const binormal = gameState.warpFrames.binormals[frameIndex];
       
       // Apply stable up vector before lookAt to prevent flipping
       playerGroup.up.copy(normal);

       const pt = gameState.warpCurve.getPointAt(t);
       const lookPt = gameState.warpCurve.getPointAt(t + 0.01);
       
       // 2. Base Player Orientation: Place at point, look down the curve
       // By setting the up vector to normal, the player perfectly aligns with the tube walls
       playerGroup.position.copy(pt);
       playerGroup.lookAt(lookPt);
       
       // 3. Screen-Relative Steering!
       // Use tubeX (left/right) and tubeY (up/down) inside the tunnel
       if (gameState.tubeX === undefined) {
           gameState.tubeX = 0;
           gameState.tubeY = 0;
       }
       ...
       let dropSpeed = 1.0;
       let rawX = 0, rawY = 0;
       if (gameState.keys.w || gameState.keys.ArrowUp) { rawY += dropSpeed; } // W goes UP on screen
       if (gameState.keys.s || gameState.keys.ArrowDown) { rawY -= dropSpeed; } // S goes DOWN on screen
       if (gameState.keys.d || gameState.keys.ArrowRight) { rawX += dropSpeed; } // D goes RIGHT on screen
       if (gameState.keys.a || gameState.keys.ArrowLeft) { rawX -= dropSpeed; } // A goes LEFT on screen
       
       gameState.tubeX += rawX;
       gameState.tubeY += rawY;

       // Keep player inside the wormhole tube (Radius is 35, limit to 25)
       const dist = Math.sqrt(gameState.tubeX**2 + gameState.tubeY**2);
       if (dist > 25) {
           const angle = Math.atan2(gameState.tubeY, gameState.tubeX);
           gameState.tubeX = Math.cos(angle) * 25;
           gameState.tubeY = Math.sin(angle) * 25;
       }

       // 4. Apply the relative offset using local axes!
       playerGroup.translateX(gameState.tubeX);
       playerGroup.translateY(gameState.tubeY);

       // 5. Epic Third Person Falling Camera!
       // Place camera rigidly relative to the final player position
       camera.position.copy(playerGroup.position);
       
       // Translate the camera slightly above and behind the player's back
       // We use the playerGroup's matrix to know where "behind" and "above" is!
       const forward = new THREE.Vector3(0, 0, 1).applyQuaternion(playerGroup.quaternion); // +Z is behind in ThreeJS
       const up = new THREE.Vector3(0, 1, 0).applyQuaternion(playerGroup.quaternion); // +Y is above
       
       camera.position.add(up.multiplyScalar(4)); // Tighter height
       camera.position.add(forward.multiplyScalar(5)); // Tighter follow distance
       
       // 6. Look perfectly down the tube into the curve!
       let camLookT = t + 0.03;
       if (camLookT > 1.0) camLookT = 1.0;
       
       const camLookPt = gameState.warpCurve.getPointAt(camLookT);
       const camLookFrameIndex = Math.min(Math.floor(camLookT * 200), 200);
       const camLookNormal = gameState.warpFrames.normals[camLookFrameIndex];
       
       // Set camera up vector to prevent gimbal flip and match tunnel orientation
       camera.up.copy(camLookNormal);
       camera.lookAt(camLookPt);
       
       // Pitch slightly down so character is at bottom of screen
       camera.rotateX(-0.1);

       // 7. Smoothly bank and lean the character model for that "flying" feel!
       const model = playerGroup.children.find(c => c.type === 'Group' && c !== gameState.warpTunnel);
       if (model) {
           const targetBank = -rawX * 0.4;
           const targetPitch = rawY * 0.4;
           model.rotation.z += (targetBank - model.rotation.z) * 0.15;
           model.rotation.x += (targetPitch - model.rotation.x) * 0.15;
       }

       // Very subtle head bobbing/falling jitter on the camera itself
  ...
  // Normal Third Person calculation
  const offsetX = Math.sin(gameState.yaw) * Math.cos(gameState.pitch) * distance;
  const offsetY = -Math.sin(gameState.pitch) * distance;
  const offsetZ = Math.cos(gameState.yaw) * Math.cos(gameState.pitch) * distance;

  const targetThirdPersonX = playerGroup.position.x + offsetX;
  const targetThirdPersonY = playerGroup.position.y + heightOffset + offsetY;
  const targetThirdPersonZ = playerGroup.position.z + offsetZ;
  
  const targetLookX = playerGroup.position.x;
  const targetLookY = playerGroup.position.y + 1.5;
  const targetLookZ = playerGroup.position.z;

  if (gameState.isCinematicLying || gameState.cinematicCameraBlend > 0) {
      // First Person POV while lying on ground
      const headY = 0.3; // Very close to floor
      const firstPersonX = playerGroup.position.x;
      const firstPersonY = playerGroup.position.y + headY;
      const firstPersonZ = playerGroup.position.z + 1.0; // Slightly forward
      
      const blend = gameState.cinematicCameraBlend; // 1.0 = pure First Person, 0.0 = pure Third Person
      
      camera.position.x = firstPersonX * blend + targetThirdPersonX * (1 - blend);
      camera.position.y = firstPersonY * blend + targetThirdPersonY * (1 - blend);
      camera.position.z = firstPersonZ * blend + targetThirdPersonZ * (1 - blend);
      
      const lookDist = 10;
      const firstLookX = playerGroup.position.x + Math.sin(gameState.cinematicPan) * lookDist;
      // Use cinematicLookUp variable so we can animate from the sky (100) down to the room (3.0)
      const firstLookY = playerGroup.position.y + headY + gameState.cinematicLookUp + Math.sin(gameState.cinematicPan * 2) * 2.0; 
      const firstLookZ = playerGroup.position.z + Math.cos(gameState.cinematicPan) * lookDist;
      
      const blendedLookX = firstLookX * blend + targetLookX * (1 - blend);
      const blendedLookY = firstLookY * blend + targetLookY * (1 - blend);
      const blendedLookZ = firstLookZ * blend + targetLookZ * (1 - blend);
      
      camera.lookAt(blendedLookX, blendedLookY, blendedLookZ);
  } else {
      // Pure Third Person Follow
      camera.position.x = targetThirdPersonX;
      camera.position.y = targetThirdPersonY;
      camera.position.z = targetThirdPersonZ;
...
function animate() {
  try {
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.zeroGStars) {
        gameState.zeroGStars.rotation.y += 0.05 * dt;
        gameState.zeroGStars.rotation.x += 0.02 * dt;
      }
      
      // Solar System Animations
      if (gameState.solarSystem) {
          // Rotate Sun
          if (gameState.zeroGSun) {
            gameState.zeroGSun.rotation.y += 0.05 * dt;
          }
          ...
          // Animate Planets
          if (gameState.zeroGPlanets) {
              gameState.zeroGPlanets.forEach(p => {
                  if (p.suckedIn) return;
                  
                  // Orbit logic if no gravity pull
                  if (gameState.gravityPull === 0) {
                      p.angle += p.speed * dt;
                      p.mesh.position.x = Math.cos(p.angle) * p.distance;
                      p.mesh.position.z = Math.sin(p.angle) * p.distance;
                      p.mesh.rotation.y += p.speed * 2 * dt; // Spin on axis
                  } else if (gameState.wormholeEntrance) {
                      // RIP PLANET OUT OF ORBIT AND SUCK IT IN
                      p.orbitLine.visible = false; // Hide orbit line
                      
                      const planetWorldPos = new THREE.Vector3();
                      p.mesh.getWorldPosition(planetWorldPos);
                      
                      const pullDir = new THREE.Vector3().subVectors(whWorldPos, planetWorldPos);
                      const distToWH = pullDir.length();
                      pullDir.normalize();
                      
                      const gravityIntensity = (gameState.gravityPull * 5.0) * (1.0 + (300 / Math.max(distToWH, 1.0)));
                      
                      // Move mesh local position based on world pull direction (since scale is 1 it maps cleanly)
                      p.mesh.position.x += pullDir.x * gravityIntensity;
                      p.mesh.position.y += pullDir.y * gravityIntensity;
                      p.mesh.position.z += pullDir.z * gravityIntensity;
          ...
          // Animate Meteors
          if (gameState.zeroGMeteors) {
              gameState.zeroGMeteors.forEach(mGroup => {
                  if (!mGroup.userData.active) {
                      // 1% chance to spawn
                      if (Math.random() < 0.01) {
                          mGroup.userData.active = true;
                          // Spawn at edge of system
                          const dist = 800 + Math.random() * 400;
                          const angle = Math.random() * Math.PI * 2;
                          mGroup.position.set(Math.cos(angle) * dist, (Math.random() - 0.5) * 200, Math.sin(angle) * dist);
                          // Aim across the system
                          mGroup.lookAt((Math.random() - 0.5) * 400, 0, (Math.random() - 0.5) * 400);
                          mGroup.userData.speed = 200 + Math.random() * 200;
                          mGroup.visible = true;
                      }
                  } else {
                      if (gameState.gravityPull === 0) {
                          // Fly forward
                          mGroup.translateZ(mGroup.userData.speed * dt);
                          if (mGroup.position.length() > 1500) {
                              mGroup.userData.active = false;
                          }
                      } else if (gameState.wormholeEntrance) {
                          // SUCK IN METEOR
                          const mWorldPos = new THREE.Vector3();
                          mGroup.getWorldPosition(mWorldPos);
                          
                          const pullDir = new THREE.Vector3().subVectors(whWorldPos, mWorldPos);
                          const distToWH = pullDir.length();
                          pullDir.normalize();
                          
                          const gravityIntensity = (gameState.gravityPull * 10.0) * (1.0 + (300 / Math.max(distToWH, 1.0)));
                          
                          mGroup.position.x += pullDir.x * gravityIntensity;
                          mGroup.position.y += pullDir.y * gravityIntensity;
                          mGroup.position.z += pullDir.z * gravityIntensity;
                          
                          // Look at wormhole as it gets sucked in
                          mGroup.lookAt(gameState.wormholeEntrance.position);
                          
                          if (distToWH < 100) {
                              mGroup.visible = false;
                              mGroup.userData.active = false; // reset
                          }
                      }
                  }
              });
      ...
      if (gameState.wormholeEntrance) {
         gameState.wormholeEntrance.rotation.y += 2.0 * dt;
         gameState.wormholeEntrance.rotation.x += 1.0 * dt;
         if (gameState.wormholeAura) {
            // Pulse aura
            const scale = 1.0 + Math.sin(gameState.bobTime * 5.0) * 0.2;
            gameState.wormholeAura.scale.set(scale, scale, scale);
         }
      }
      ...
      // Animate Shader Wormhole during drop
      if (gameState.warpTunnel && gameState.isDropping) {
        // Update shader time for plasma animation
        if (gameState.warpTunnelMat) {
           gameState.warpTunnelMat.uniforms.time.value += dt;
        }

        // Animate dust particles rushing upwards
        if (gameState.warpDust) {
           const positions = gameState.warpDust.geometry.attributes.position.array;
           for(let i=0; i<positions.length; i+=3) {
              positions[i+1] += 1200 * dt; // Rushing upwards faster
              if (positions[i+1] > 2400) positions[i+1] -= 2400; // Loop vertically across the longer tube
           }
           gameState.warpDust.geometry.attributes.position.needsUpdate = true;
        }
      }
