      ...
      gsap.to(flash, { intensity: 0, duration: 0.5, ease: "power2.out", onComplete: () => {
        ...
        if (gameState.pulses) {
          gameState.pulses.forEach(p => {
            p.progress = 0;
            p.speed = 20 + Math.random() * 5; // Fast, visible surge
          });
        }
      }});
...
function activateEnvironment() {
  ...
  // Because pulses are delayed by 0.5s due to the flash, we add 0.5 to the delay here
  let tIndex = 0;
  terminals.forEach(t => {
    if (t.userData.light) {
      // Pulse travel time is about ~0.75s, plus 0.5s flash delay = 1.25s
      gsap.to(t.userData.light, { intensity: 1.0, duration: 0.5, delay: 1.2 + (tIndex * 0.2) });
      if (t.material) {
        t.material.opacity = 0; // Start completely off
        gsap.to(t.material, { opacity: 0.15, duration: 0.5, delay: 1.2 + (tIndex * 0.2) });
      }
      tIndex++;
    }
  });
  ...
  if (gameState.dirLight) gsap.to(gameState.dirLight, { intensity: 1.5, duration: 3, delay: 2.0 });
  if (gameState.hemiLight) gsap.to(gameState.hemiLight, { intensity: 0.4, duration: 3, delay: 2.0 });
}
