...
function updateMovement() {
  ...
  if (gameState.isDropping) {
    if (gameState.warpCurve) {
       ...
       // 5. Epic Spline-Riding Camera!
       // Place the camera slightly behind the player on the spline, with the SAME tube offset!
       const camT = Math.max(0, t - 0.025);
       const camPt = gameState.warpCurve.getPointAt(camT);
       const camFrameIndex = Math.min(Math.floor(camT * 200), 200);
       const camNormal = gameState.warpFrames.normals[camFrameIndex];
       
       // Use a dummy object to perfectly calculate the offset position relative to the curve at the camera's location
       const camDummy = new THREE.Object3D();
       camDummy.position.copy(camPt);
       camDummy.up.copy(camNormal);
       camDummy.lookAt(gameState.warpCurve.getPointAt(Math.min(1.0, camT + 0.01)));
       
       // Translate the camera to match the player's cross-section, plus float slightly above them
       camDummy.translateX(gameState.tubeX);
       camDummy.translateY(gameState.tubeY + 5.0); 
       
       camera.position.copy(camDummy.position);
       camera.up.copy(camNormal); // Ensure camera has stable up vector
       
       // 6. Look perfectly down the tube, matching the player's offset
       const lookT = Math.min(1.0, t + 0.05);
       const lookPt = gameState.warpCurve.getPointAt(lookT);
       const lookFrameIndex = Math.min(Math.floor(lookT * 200), 200);
       
       const lookDummy = new THREE.Object3D();
       lookDummy.position.copy(lookPt);
       lookDummy.up.copy(gameState.warpFrames.normals[lookFrameIndex]);
       lookDummy.lookAt(gameState.warpCurve.getPointAt(Math.min(1.0, lookT + 0.01)));
       
       // Apply the exact same tube offsets to the look target so our gaze is perfectly parallel to the tunnel walls!
       lookDummy.translateX(gameState.tubeX);
       lookDummy.translateY(gameState.tubeY);
       
       camera.lookAt(lookDummy.position);
