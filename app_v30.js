...
window.enterMissionControl = function enterMissionControl() {
  ...
  if (playerGroup) {
    playerGroup.position.y = 800;
    // We now keep the player visible during the drop for an epic Third-Person view!
  }
  ...
  playSound('rumble');
  setTimeout(() => playSound('whoosh'), 1000);
  setTimeout(() => playSound('whoosh'), 3000);
  setTimeout(() => playSound('whoosh'), 4500);

  // Animate RGB Shift Intensity
  if (gameState.rgbShift) {
    gameState.rgbShift.enabled = true;
    gameState.rgbShift.uniforms['amount'].value = 0.0;
    gsap.to(gameState.rgbShift.uniforms['amount'], {
      value: 0.02, // Intense split
      duration: 5.5,
      ease: "power2.in"
    });
  }
  ...
  gsap.to(playerGroup.position, {
    ...
    onComplete: () => {
      ...
      if (gameState.warpTunnel) {
        ...
        if (gameState.warpCylinder) {
          gameState.warpCylinder.geometry.dispose();
          gameState.warpCylinder.material.dispose();
        }
...
function buildWarpTunnel() {
  ...
  // 1. A clean, straight, continuous neon grid wormhole with ANIMATED TEXTURE for true flight!
  ...
  const warpTex = new THREE.CanvasTexture(canvas);
  warpTex.wrapS = THREE.RepeatWrapping;
  warpTex.wrapT = THREE.RepeatWrapping;
  warpTex.repeat.set(8, 100); // Repeat grid along the long cylinder

  const tubeGroup = new THREE.Group();
  const cylinderGeo = new THREE.CylinderGeometry(20, 20, 2000, 32, 1, true); // Open ended
  const cylinderMat = new THREE.MeshBasicMaterial({ 
    map: warpTex, 
    transparent: true, 
    opacity: 0.9,
    side: THREE.BackSide // Render inside the tube
  });
  const tunnelMesh = new THREE.Mesh(cylinderGeo, cylinderMat);
  tubeGroup.add(tunnelMesh);
  tunnelGroup.add(tubeGroup);

  // 2. High-speed warp stars (long streaking boxes)
  const starsGroup = new THREE.Group();
  const starGeo = new THREE.BoxGeometry(0.05, 40, 0.05); // 40 units long for hyper-streak effect
  const starMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9 });
  
  const starCount = 400;
  for(let i=0; i<starCount; i++) {
    const star = new THREE.Mesh(starGeo, starMat);
    const r = 4 + Math.random() * 3.5; // Inside the tunnel (radius 8)
    const theta = Math.random() * Math.PI * 2;
    const y = (Math.random() - 0.5) * 2000;
    star.position.set(Math.cos(theta) * r, y, Math.sin(theta) * r);
    starsGroup.add(star);
  }
  tunnelGroup.add(starsGroup);

  // 3. Glowing Time Rings
  const ringCount = 20;
  const rings = new THREE.Group();
  const timeRingGeo = new THREE.TorusGeometry(7.5, 0.1, 8, 32);
  const timeRingMat = new THREE.MeshBasicMaterial({ color: 0xaa00ff, transparent: true, opacity: 0.8 });
  for(let i=0; i<ringCount; i++) {
    const ring = new THREE.Mesh(timeRingGeo, timeRingMat);
    ring.position.y = (Math.random() - 0.5) * 2000;
    ring.rotation.x = Math.PI / 2;
    rings.add(ring);
  }
  tunnelGroup.add(rings);
  
  // 4. Lightning removed as requested

  // Attach to player so it falls with them
  playerGroup.add(tunnelGroup);
  
  gameState.warpTunnel = tunnelGroup;
  gameState.warpTunnelMat = cylinderMat;
  gameState.warpTubeGroup = tubeGroup;
  gameState.warpStars = starsGroup;
  gameState.warpRings = rings;
  gameState.warpLightnings = [];
}
...
function updateMovement() {
  ...
  if (gameState.isDropping) {
    // Allow user to steer inside the wormhole!
    let dropSpeed = 1.5;
    let dx = 0, dz = 0;
    ...
    // Keep player inside the wormhole tube (Radius is 20)
    ...
    if (dist > 18) {
       ...
       playerGroup.position.x = Math.cos(angle) * 18;
       playerGroup.position.z = Math.sin(angle) * 18;
    }

    // Epic Third Person Falling Camera!
    // Lock the camera tightly to the player so it feels perfectly in sync and responsive
    camera.position.x = playerGroup.position.x;
    camera.position.y = playerGroup.position.y + 5; // Over the shoulder
    camera.position.z = playerGroup.position.z + 6; // Just behind
    
    // Look straight past the player down into the massive tunnel
    camera.lookAt(playerGroup.position.x, playerGroup.position.y - 40, playerGroup.position.z);
    
    // Smoothly bank and lean the character model for that "flying" feel!
    // We find the loaded GLTF model inside the player group
    const model = playerGroup.children.find(c => c.type === 'Group' && c !== gameState.warpTunnel);
    if (model) {
       const targetBank = -dx * 0.4;
       const targetPitch = dz * 0.4;
  ...
  try {
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.warpTunnel && gameState.isDropping) {
        // Spin the tube group rapidly for hyperspace effect
        gameState.warpTubeGroup.rotation.y += 3.0 * dt;

        // Move the grid texture backwards to simulate insane forward flight speed!
        if (gameState.warpTunnelMat && gameState.warpTunnelMat.map) {
          gameState.warpTunnelMat.map.offset.y -= 15.0 * dt;
        }

        // Move stars UP relative to player (camera looks DOWN)
        gameState.warpStars.children.forEach(star => {
          star.position.y += 3000 * dt; 
          if (star.position.y > 1000) star.position.y -= 2000;
        });

        // Rings move UP relative to player
        gameState.warpRings.children.forEach(ring => {
          ring.position.y += 1800 * dt;
          if (ring.position.y > 1000) ring.position.y -= 2000;
        });

        // Flashing plasma lightning
        gameState.warpLightnings.forEach(l => {
          if (!l.active && Math.random() > 0.95) {
             l.active = true;
             l.timer = 0.15; // flashes for 0.15 seconds
             l.mesh.position.y = (Math.random()-0.5)*2000;
             l.mesh.material.opacity = 0.9;
             // Random color (white or bright cyan)
             l.mesh.material.color.setHex(Math.random() > 0.5 ? 0xffffff : 0x00aaff);
          }
          if (l.active) {
             l.timer -= dt;
             // Erratic scaling to look like a blast
             l.mesh.scale.set(1 + Math.random()*2, 1 + Math.random()*5, 1 + Math.random()*2);
             if (l.timer <= 0) {
                l.active = false;
