...
function enterZeroGSpace() {
  ...
  // Dynamic Wormhole Opening (Delayed by 3 seconds)
  setTimeout(() => {
    // Create Wormhole Entrance Marker
    const whGeo = new THREE.SphereGeometry(20, 32, 32);
    ...
    // Position it exactly 80 units in front of where they spawned
    gameState.wormholeEntrance.position.set(DEEP_SPACE_COORD, DEEP_SPACE_COORD, DEEP_SPACE_COORD - 80); 
    
    // Scale starts at 0
    gameState.wormholeEntrance.scale.set(0, 0, 0);
    scene.add(gameState.wormholeEntrance);

    // Add a massive glowing aura to the entrance
    const auraGeo = new THREE.SphereGeometry(30, 32, 32);
    ...
    // Animate opening!
    playSound('beep'); // Sound of anomaly opening
    gsap.to(gameState.wormholeEntrance.scale, { x: 1, y: 1, z: 1, duration: 2.5, ease: "elastic.out(1, 0.5)", onComplete: () => {
        // Prompt the user ONLY after it opens
        const prompt = document.getElementById('interaction-prompt');
        if (prompt) {
          prompt.style.opacity = 1;
          prompt.innerHTML = `WORMHOLE ANOMALY DETECTED - USE <span>MOUSE</span> & <span>WASD</span> TO ENTER`;
        }
    }});
  }, 3000);
}
