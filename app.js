...
window.enterMissionControl = function enterMissionControl() {
  ...
  gsap.to(playerGroup.position, {
    ...
    onComplete: () => {
      ...
      scene.add(flash);
      gsap.to(flash, { intensity: 0, duration: 1.5, ease: "power2.out", onComplete: () => scene.remove(flash) });
      ...
      // Reset Data Pulses to burst from the center!
      if (gameState.pulses) {
        gameState.pulses.forEach(p => p.progress = 0);
      }
...
function activateEnvironment() {
  ...
  // Terminals turn on SEQUENTIALLY after a delay (simulating the pulse wave hitting them)
  let tIndex = 0;
  terminals.forEach(t => {
    if (t.userData.light) {
      // Delay syncs with pulse travel time + sequential stagger
      gsap.to(t.userData.light, { intensity: 1.0, duration: 0.5, delay: 0.7 + (tIndex * 0.2) });
      if (t.material) {
        t.material.opacity = 0; // Start completely off
        gsap.to(t.material, { opacity: 0.15, duration: 0.5, delay: 0.7 + (tIndex * 0.2) });
      }
      tIndex++;
    }
  });
  ...
  if (gameState.dirLight) gsap.to(gameState.dirLight, { intensity: 1.5, duration: 3, delay: 1.5 });
  if (gameState.hemiLight) gsap.to(gameState.hemiLight, { intensity: 0.4, duration: 3, delay: 1.5 });
}
        ...
        gameState.pulses.forEach(pulseData => {
          ...
          for(let i=0; i<pulseData.distances.length-1; i++) {
             if (pulseData.progress >= pulseData.distances[i] && pulseData.progress < pulseData.distances[i+1]) {
                ...
                // Lerp position along the line segment
