...
window.enterMissionControl = function enterMissionControl() {
  ...
  gsap.to(gameState, {
    ...
    onComplete: () => {
      ...
      playerGroup.position.set(0, 0, 15); // Hard snap to exact floor coordinate
      playerGroup.up.set(0, 1, 0); // RESTORE DEFAULT UP VECTOR!
      ...
      playerGroup.children.forEach(child => {
        child.visible = true; // Show model again
        // Reset the model's internal banking/pitching from the flight phase
        if (child.type === 'Group' && child !== gameState.warpTunnel) {
...
function buildWarpTunnel() {
  ...
  // Ensure the last few points straighten out into the room
  points[38] = new THREE.Vector3(0, 150, 15);
  points[39] = new THREE.Vector3(0, 50, 15);
  points[40] = new THREE.Vector3(0, 0, 15);
