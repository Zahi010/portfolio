...
function buildWarpTunnel() {
  ...
  for (let i = 0; i < pos.length; i += 3) {
    ...
    const dx = Math.sin(y * 0.005) * 12;
    const dz = Math.cos(y * 0.005) * 12;
  ...
  const mat = new THREE.MeshStandardMaterial({
    ...
    emissive: 0x002288,     // Deep blue ambient glow
    emissiveIntensity: 0.2, // Low emissive so lightning contrast is huge
    wireframe: true,
    transparent: true,
    opacity: 0.9,
  ...
  try {
    ...
    if (gameState.isBooted) {
      ...
      if (gameState.warpTunnel && gameState.isDropping) {
        // Spin the cylinder
        gameState.warpCylinder.rotation.y += 5 * dt;   
