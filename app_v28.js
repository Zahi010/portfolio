...
function updateMovement() {
  ...
  if (gameState.isDropping) {
    ...
    // Camera is placed very close to the character's back so it feels like they are flying forward
    const targetCamPos = new THREE.Vector3(
       playerGroup.position.x,
       playerGroup.position.y + 4, // Just slightly above the head
       playerGroup.position.z + 4  // Right behind the back
    );
    ...
    // Add dynamic camera tilt based on movement for visceral feeling
    camera.rotation.z += -dx * 0.02;
    camera.rotation.x += dz * 0.02;

    // Head bobbing/falling jitter
