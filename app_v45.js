...
function enterZeroGSpace() {
  ...
  // Generate Local Deep Space Starfield
  const starGeo = new THREE.BufferGeometry();
  const starCount = 3000;
  const starPos = new Float32Array(starCount * 3);
  for(let i=0; i<starCount*3; i+=3) {
    const radius = 200 + Math.random() * 800; // between 200 and 1000 units away
  ...
  const sunMat = new THREE.MeshBasicMaterial({ color: 0xffaa00, transparent: true, opacity: 0.9 });
  gameState.zeroGSun = new THREE.Mesh(sunGeo, sunMat);
  gameState.zeroGSun.position.set(DEEP_SPACE_COORD, DEEP_SPACE_COORD, DEEP_SPACE_COORD - 200);
  scene.add(gameState.zeroGSun);
  
  // Actually emit light into the Solar System!
  const sunLight = new THREE.PointLight(0xffaa00, 2.0, 2000); // Color, intensity, distance
  gameState.zeroGSun.add(sunLight);
  
  // Sun Aura
  const sunAuraGeo = new THREE.SphereGeometry(80, 32, 32);
  const sunAuraMat = new THREE.MeshBasicMaterial({ color: 0xff4400, transparent: true, opacity: 0.4, blending: THREE.AdditiveBlending, depthWrite: false });
  ...
  const planetMat = new THREE.MeshStandardMaterial({ color: 0x2244ff, roughness: 0.8 });
  gameState.zeroGPlanet = new THREE.Mesh(planetGeo, planetMat);
  gameState.zeroGPlanet.position.set(DEEP_SPACE_COORD - 300, DEEP_SPACE_COORD + 100, DEEP_SPACE_COORD - 100);
  scene.add(gameState.zeroGPlanet);
  
  // Asteroid Field
  gameState.zeroGAsteroids = [];
  ...
  for (let i = 0; i < 40; i++) {
     ...
     // Randomly around the player
     ast.position.set(
        DEEP_SPACE_COORD + (Math.random() - 0.5) * 100,
        DEEP_SPACE_COORD + (Math.random() - 0.5) * 100,
        DEEP_SPACE_COORD + (Math.random() - 0.5) * 100
     );
     // Give them a random spin velocity
     ast.userData = {
         rx: Math.random() * 0.05,
         ry: Math.random() * 0.05,
         rz: Math.random() * 0.05
     ...
     scene.add(ast);
     gameState.zeroGAsteroids.push(ast);
  }
  ...
  // Phase 2: Supernova Flash (t=4.0s)
  ...
  timeline.add(() => {
     ...
     // Position exactly where the Sun was
     gameState.wormholeEntrance.position.set(DEEP_SPACE_COORD, DEEP_SPACE_COORD, DEEP_SPACE_COORD - 200); 
     gameState.wormholeEntrance.scale.set(0, 0, 0);
     scene.add(gameState.wormholeEntrance);
     ...
     gsap.to(gameState, { gravityPull: 3.0, duration: 8.0, ease: "power2.in" });
...
window.enterMissionControl = function enterMissionControl() {
  ...
  if (gameState.wormholeEntrance) {
      scene.remove(gameState.wormholeEntrance);
      gameState.wormholeEntrance.geometry.dispose();
      gameState.wormholeEntrance.material.dispose();
      gameState.wormholeEntrance = null;
  }
  ...
  if (gameState.zeroGPlanet) {
      scene.remove(gameState.zeroGPlanet);
      gameState.zeroGPlanet.geometry.dispose();
      gameState.zeroGPlanet.material.dispose();
      gameState.zeroGPlanet = null;
  }
  if (gameState.zeroGAsteroids) {
      gameState.zeroGAsteroids.forEach(ast => {
         scene.remove(ast);
         ast.geometry.dispose();
         ast.material.dispose();
      });
      gameState.zeroGAsteroids = null;
  }
...
function updateMovement() {
  ...
  if (gameState.isZeroG) {
      ...
      if (gameState.wormholeEntrance && gameState.gravityPull > 0) {
          const whPos = gameState.wormholeEntrance.position;
          
          // Calculate direction from player to wormhole
          const pullDir = new THREE.Vector3().subVectors(whPos, playerGroup.position);
          const distToWH = pullDir.length();
          pullDir.normalize();
          
          // Add gravity vector to movement
          // Gravity is stronger the closer you are!
          const gravityIntensity = gameState.gravityPull * (1.0 + (100 / Math.max(distToWH, 1.0)));
      ...
      if (gameState.wormholeEntrance) {
          const dist = playerGroup.position.distanceTo(gameState.wormholeEntrance.position);
          if (dist < 40) { // Increased hit radius since the wormhole is bigger
              enterMissionControl();
          }
      }
...
function animate() {
  try {
    ...
    if (gameState.isBooted) {
      ...
      // Rotate Sun slowly
      if (gameState.zeroGSun) {
        gameState.zeroGSun.rotation.y += 0.1 * dt;
      }
      
      // Animate asteroids floating and getting sucked in
      if (gameState.zeroGAsteroids && gameState.zeroGAsteroids.length > 0) {
         gameState.zeroGAsteroids.forEach(ast => {
             // Spin
             ast.rotation.x += ast.userData.rx;
             ast.rotation.y += ast.userData.ry;
             ast.rotation.z += ast.userData.rz;
             
             // Gravity Pull
             if (gameState.wormholeEntrance && gameState.gravityPull > 0) {
                 const whPos = gameState.wormholeEntrance.position;
                 const pullDir = new THREE.Vector3().subVectors(whPos, ast.position);
                 const distToWH = pullDir.length();
                 pullDir.normalize();
                 
                 // Sucked in faster than player because they have no thrust!
                 const gravityIntensity = (gameState.gravityPull * 1.5) * (1.0 + (100 / Math.max(distToWH, 1.0)));
                 ast.position.x += pullDir.x * gravityIntensity;
                 ast.position.y += pullDir.y * gravityIntensity;
                 ast.position.z += pullDir.z * gravityIntensity;
