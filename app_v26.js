...
window.enterMissionControl = function enterMissionControl() {
  ...
  if (playerGroup) {
    ...
    // Hide ONLY the player model, not the entire group!
...
function updateMovement() {
  ...
  if (gameState.isDropping) {
    ...
    // Look straight down the time portal, with a slight z-offset to prevent gimbal lock
    camera.lookAt(playerGroup.position.x, playerGroup.position.y - 10, playerGroup.position.z + 0.001);
    
    // Add dynamic camera tilt based on movement for visceral feeling
    camera.rotation.z += -dx * 0.05;
    camera.rotation.x += dz * 0.05;
